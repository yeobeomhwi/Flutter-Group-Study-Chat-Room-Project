...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> updateReservationStatus(String docId) async {
    try {
      // 문서 스냅샷 가져오기
      final docSnapshot = await _firestore.collection('study_rooms')
          .doc(docId)
          .get();
      ...
      if (docSnapshot.exists) {
        ...
        final reservations = docSnapshot.data()?['reservations'] as Map<
            String,
            dynamic>?;

        // 현재 사용자의 예약 상태를 확인
        if (reservations != null) {
          final currentUserReservation = reservations[currentUserId] ??
              false; // 현재 사용자의 예약 상태 가져오기
