...
class _CreateRoomPageState extends State<CreateRoomPage> {
  ...
  final List<String> _topics = [ // 주제 리스트
    '수학',
    '영어 회화',
    '토익',
    '프로그래밍',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                print('Start Date: $_startDate, Start Time: $_startTime'); // 선택된 시작 시간 로그
                print('End Date: $_endDate, End Time: $_endTime'); // 선택된 종료 시간 로그
                int? maxParticipants =
                int.tryParse(_maxParticipantsController.text); // 최대 인원 숫자로 변환
                if (maxParticipants != null &&
                    _startDate != null &&
                    _endDate != null &&
                    _startTime != null &&
                    _endTime != null) { // 모든 시간 변수가 유효한지 확인
                  try {
                    // 방 생성
                    await _createRoom.createStudyRoom(
                      title: _titleController.text, // 방 제목
                      topic: _selectedTopic ?? '', // 주제가 선택되지 않았다면 빈 문자열
                      content: _contentController.text, // 방 설명
                      ...
                      endTime: tz.TZDateTime(
                          ...
                          _endTime!.minute)
                          .toDate(), // DateTime으로 변환
                      maxParticipants: maxParticipants, // 최대 인원
                    );

                    // 방 생성 완료 후 알림
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('방이 생성되었습니다!')), // 방 생성 성공 메시지
                    );
                  ...
                  // 입력값이 유효하지 않을 경우 경고
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('모든 필드를 올바르게 입력해주세요.')), // 입력 오류 메시지
                  );
                }
              },
